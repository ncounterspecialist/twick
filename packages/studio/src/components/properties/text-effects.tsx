...
export function TextEffects({ selectedElement, isOpen, updateElement, onToggle }: TextEffectsProps & PropertiesPanelProps) {
  ...
  const handleUpdateEffect = (props: {
    ...
    if (!effect || (props.name && props.name !== effect.getName())) {
      effect = new ElementTextEffect(props.name || currentEffect?.getName() || TEXT_EFFECTS[0].name);
